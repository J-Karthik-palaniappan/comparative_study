...
def process_and_evaluate(
    ...
    for path in paths_to_process:
        ...
        # Original mech does not use landmarks and iters
        ...
        cv2.imwrite(f"{save_dir}/{imgname}_{args.mech}.png", output_np)
        ...
        if img_gt is not None:
            ...
            test_results["ssim_y"].append(ssim_y_val)
    ...
    print(f"PSNR:{ave_psnr:.2f}, SSIM:{ave_ssim:.4f}, PSNR_Y:{ave_psnr_y:4f}, SSIM_Y:{ave_ssim_y:4f}")
...
def define_model(args, mech, num_landmarks, iters, window_size, img_size, **kwargs):
    ...
    # 1. Save initial parameters (clone for safety)
    ...
    # 3. Compare parameters and report which loaded, which remained random
    # loaded_keys = []
    # random_keys = []

    # for k, v in model.state_dict().items():
    #     if k in init_state:
    #         if torch.allclose(v, init_state[k]):   # unchanged → random init
    #             random_keys.append(k)
    #         else:                                  # different → loaded from checkpoint
